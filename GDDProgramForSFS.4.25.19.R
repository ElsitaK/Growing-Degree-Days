...
setwd('~/Documents/NCSU/Machine Learning CH4/MACA RCP8.5 A2/MODIFIED Tmax Tmin')
...
for (i in files){
  ...
  filename <- file.path('~/Documents/NCSU/Machine Learning CH4/MACA RCP8.5 A2/GDD all 20 GCMs', paste0("tasmin", "_", names(temperatures)[3], "csv"))
  ...
  #new column with formula calculation per day
  ...
  #get the number of days per each year, put it in a table
  DaysPerYear <- temperatures %>% count(Year)
  DaysPerYear[85,1] <- 2100 #add this to avoid trivial errors later that will stop for loop
  DaysPerYear[85,2] <- 366 #add this to avoid trivial errors later
